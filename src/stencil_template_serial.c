...
int main(int argc, char **argv)
{
  ...
  double t_get_total_energy = 0.0;
  double t_update_plane = 0.0;
  double t_inject_energy = 0.0;
  double t0;
  ...
  const double t_wall = wtime() - t_wall_start;
  const double updates = (double)S[_x_] * (double)S[_y_] * (double)Niterations;
  const double glups = updates / t_update_plane / 1e9;

  printf("injected energy is %g, system energy is %g\n",
	 injected_heat, system_heat );

  printf("t_update_plane total      : %.6f s\n", t_update_plane);
  printf("t_inject_energy total      : %.6f s\n", t_inject_energy);
  printf("t_get_total_energy total   : %.6f s\n", t_get_total_energy);
  printf("t_wall total               : %.6f s\n", t_wall);
  printf("performance                : %.6f GLUP/s\n", glups);
  printf("CSV,%.6f,%.6f,%.6f,%.6f,%.6f\n",
         t_wall, t_get_total_energy, t_update_plane, t_inject_energy, glups);
