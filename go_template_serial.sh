#!/bin/bash
#SBATCH --job-name=stencil_template_serial
#SBATCH --nodes=1
...
RUN_DIR=${RESULTS_ROOT}/${SLURM_JOB_ID}_template_serial
RAW_DIR=${RUN_DIR}/raw
JOB_BUILD_DIR=${RUN_DIR}/build
SUMMARY=${RUN_DIR}/go_template_serial.csv
...
make BUILD_DIR="${JOB_BUILD_DIR}" template-serial 2>&1 | tee "${RUN_DIR}/build.log"

printf 'run_name,t_wall,t_get_total_energy,t_update_plane,t_inject_energy,glups\n' >"${SUMMARY}"

for optimization in O1 O3; do
    executable=${JOB_BUILD_DIR}/stencil_template_serial_${optimization}
    for repetition in $(seq 1 "${REPEATS}"); do
        run_name=template_${optimization}_rep$(printf '%02d' "${repetition}")
        save_run "${run_name}" \
