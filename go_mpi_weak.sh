#!/bin/bash
#SBATCH --job-name=stencil_mpi_weak
#SBATCH --nodes=2
...
RUN_DIR=${RESULTS_ROOT}/${SLURM_JOB_ID}_mpi_weak
RAW_DIR=${RUN_DIR}/raw
JOB_BUILD_DIR=${RUN_DIR}/build
SUMMARY=${RUN_DIR}/go_mpi_weak.csv
...
grid_for_tasks()
{
    case "$1" in
        ...
        64) GRID_X=$((8*BASE_SIDE));   GRID_Y=$((8*BASE_SIDE)) ;;
        *)
            echo "Unsupported weak-scaling task count: $1" >&2
            return 1
            ;;
    esac
}
...
save_run()
{
    ...
    echo "RUN ${run_name}: tasks=${tasks}, nodes=${nodes}, grid=${GRID_X}x${GRID_Y}"
    "$@" >"${logfile}" 2>&1
    ...
    printf '%s,%s,1,%s,%s,%s,%s,%s\n' "${run_name}" "${tasks}" \
        "${nodes}" "${GRID_X}" "${GRID_Y}" "$((GRID_X * GRID_Y / tasks))" \
        "${metrics}" >>"${SUMMARY}"
}

{
    ...
    echo "cells_per_rank=$((BASE_SIDE * BASE_SIDE))"
...
printf 'run_name,tasks,threads,nodes,grid_x,grid_y,cells_per_rank,t_wall,t_get_total_energy,t_update_plane,t_inject_energy,t_exchange_halos,glups,injected_energy,system_energy\n' >"${SUMMARY}"

for tasks in ${MPI_TASKS}; do
    grid_for_tasks "${tasks}"
    nodes=$(( (tasks + 31) / 32 ))
    if (( nodes > 2 )); then
    ...
    for repetition in $(seq 1 "${REPEATS}"); do
        run_name=weak_r$(printf '%03d' "${tasks}")_n$(printf '%02d' "${nodes}")_x${GRID_X}_y${GRID_Y}_rep$(printf '%02d' "${repetition}")
...
echo "Weak-scaling runs completed. Summary: ${SUMMARY}"
cat "${SUMMARY}"
